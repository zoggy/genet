...
let detect_boxes ?(modes=[`Byte;`Opt]) lablgtk2_includes conf =
  let includes =
    ["default install",
      [Filename.concat (ocaml_libdir conf) "boxes"]]
  in
  let includes =
    match ocamlfind_query conf "boxes" with
      None -> includes
    | Some s -> ("with ocamlfind", [s]) :: includes
  in
  let libs = ["lablgtk.cma" ; "lablgnomecanvas.cma" ; "boxes.cma"] in
  let f (mes, includes) mode =
    let mes = Printf.sprintf "checking for Boxes (%s) %s... "
...
let rdf_mysql = check_ocamlfind_package conf ~fail: false ~min_version: [0;7;0] "rdf.mysql";;
let rdf_postgresql = check_ocamlfind_package conf ~fail: false ~min_version: [0;7;0] "rdf.postgresql";;
let _ =
